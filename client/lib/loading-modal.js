/**
 * Loading Modal
 * A loading modal with blur background and animated status messages.
 * Used during AI enhancement for better UX.
 */
...
export function showLoadingModal({ h, root, initialMessage = '', title = '' } = {}) {
  ...
  // Spinner using Shoelace
  const spinner = h('sl-spinner', {
    class: 'loading-modal-spinner',
    style: { fontSize: '3rem', '--track-width': '4px' },
  });
  ...
  // Assemble
  modal.append(spinner, titleEl, messageEl, progressWrap, progressText);
